...
# supported expressions are of form x +/- y
def evaluate_simple_expression(expression: str):
...
def evaluate_expression_string(s: str):
    def evaluate_match(match):
        ...
        # a valid expression is of form x +/- y
        if re.fullmatch(r'\d+(\s*[\+\-]\s*\d+)*', expression):
            return str(evaluate_simple_expression(expression))
        else:
            # Return the original text if not a valid expression
            return match.group(0)

    # search for expressions of form  {****} and replace using evaluate_match
    return re.sub(r'\{([^}]*)\}', evaluate_match, s)
    ...
    try:
        try:
            ...
            v = os.path.exists(os.path.join(os.path.join(
                os.path.expanduser("~"), ".elevenclock"), settingsName))
...
def setSettings(s: str, v: bool, r: bool = True, thread=False, env: str = ""):
    ...
    try:
        ...
        globals.settingsCache[settingsName] = v
        if (v):
            open(os.path.join(os.path.join(os.path.expanduser(
                "~"), ".elevenclock"), settingsName), "w").close()
        else:
            try:
...
def isDark():
    try:
        raise DeprecationWarning(
            "This function has been deprecated and must be replaced with isWindowDark or isTaskbarDark!")
...
def openClockSettings(clockInstance):
    try:
        ...
        try:
            ...
            print(
                f"🟢 Specific clock settings for clock {id} missing, generating one...")
...
class TaskbarIconTray(QSystemTrayIcon):
    def __init__(self, app=None):
        ...
        self.show()
        self.setToolTip("ElevenClock - " +
                        _("Customize the clock on Windows 11"))
        ...
        self.reloadInternetAction = QAction(
            _("Sync time with the internet"), app)
        self.reloadInternetAction.triggered.connect(
            lambda: setSettings("ReloadInternetTime", True))
        menu.addAction(self.reloadInternetAction)
        menu.addSeparator()
        self.nameAction = QAction(
            _("ElevenClock v{0}").format(versionName), app)
        self.nameAction.setEnabled(False)
        menu.addAction(self.nameAction)
        menu.addSeparator()
        self.restartAction = QAction(_("Restart ElevenClock"), app)
        self.restartAction.triggered.connect(
            lambda: (os.startfile(sys.executable), globals.app.quit()))
        ...
        def blacklist():
            setSettingsValue("BlacklistedMonitors", getSettingsValue(
                "BlacklistedMonitors")+f"_{self.toolsMenu.screen().name()}_")

        def warnBlacklist():
            global msg
            msg = QFramelessDialog(parent=None, closeOnClick=True, xoff=self.toolsMenu.screen(
            ).geometry().x(), yoff=self.toolsMenu.screen().geometry().y())
    ...
    """)
            msg.addButton(
                _("Yes"), QDialogButtonBox.ButtonRole.ApplyRole, lambda: blacklist())
            msg.addButton(_("No"), QDialogButtonBox.ButtonRole.RejectRole)
            msg.setDefaultButtonRole(
                QDialogButtonBox.ButtonRole.ApplyRole, globals.sw.styleSheet())
        ...
        self.moveToLeftAction.triggered.connect(
            lambda: toggleClockHorPosAction())
        ...
        self.moveToTopAction.triggered.connect(
            lambda: toggleClockVerPosAction())
        ...
        self.datetimeprefs.triggered.connect(
            lambda: os.startfile('ms-settings:dateandtime'))
        if not getSettings("HideTaskManagerButton"):
            menu.addAction(self.datetimeprefs)
        self.notifprefs = QAction(_("Notification settings"), app)
        self.notifprefs.triggered.connect(
            lambda: os.startfile('ms-settings:notifications'))
        if not getSettings("HideTaskManagerButton"):
    ...
    def showMenu(self, clock):
        ...
        if clock.clockOnTheLeft:
            ...
            pos.setX(clock.screen().geometry().x() +
                     clock.screen().geometry().width()-(5)-menuWidth)

        if clock.clockOnTop:
            pos.setY(clock.screen().geometry().y()+(5)+clock.height())
        else:
            pos.setY(clock.screen().geometry().y(
            )+clock.screen().geometry().height()-clock.height()-(5)-menuHeight)
        ...
        try:
            ...
            if getSettings("ClockOnTheLeft"):
                if getSettings(f"SpecificClockOnTheRight{screenName}"):
                    self.moveToLeftAction.setText(
                        _("Restore horizontal position"))
                else:
                    self.moveToLeftAction.setText(
                        _("Move this clock to the right"))
            else:
                if getSettings(f"SpecificClockOnTheLeft{screenName}"):
                    ...
                    self.moveToLeftAction.setText(
                        _("Move this clock to the left"))

            if getSettings("ForceOnTop"):
                if getSettings(f"SpecificClockOnTheBottom{screenName}"):
                    self.moveToTopAction.setText(
                        _("Restore vertical position"))
                else:
                    self.moveToTopAction.setText(
                        _("Move this clock to the bottom"))
            else:
                if getSettings(f"SpecificClockOnTheTop{screenName}"):
                    ...
                    self.moveToTopAction.setText(
                        _("Move this clock to the top"))
            if clockInstance:
                self.monitorInfoAction.setText(_("Clock {0} on monitor {1}").format(
                    clockInstance.getClockID()[1][0], clockInstance.getClockID()[1][1]))
                self.toolsMenu.setEnabled(True)
            else:
                self.toolsMenu.setEnabled(False)
            self.individualSettings.triggered.disconnect()
            self.individualSettings.triggered.connect(
                partial(openClockSettings, clockInstance))
    ...
    def applyStyleSheet(self) -> None:
        ...
        if isTaskbarDark():
            GlobalBlur(self.contextMenu().winId(), Acrylic=True,
                       hexColor="#21212140", Dark=True)
            GlobalBlur(self.toolsMenu.winId(), Acrylic=True,
                       hexColor="#21212140", Dark=True)
            ...
            if lastMenuTheme != 1:
                ...
                self.contextMenu().setStyleSheet(f"""
                    * {{
                    ...
                    """)
        self.datetimeprefs.setIcon(
            QIcon(getPath(f"settings_{self.iconMode}.png")))
        self.individualSettings.setIcon(
            QIcon(getPath(f"settings_{self.iconMode}.png")))
        self.notifprefs.setIcon(
            QIcon(getPath(f"settings_{self.iconMode}.png")))
        ...
        self.monitorInfoAction.setIcon(
            QIcon(getPath(f"about_{self.iconMode}.png")))
        self.moveToLeftAction.setIcon(
            QIcon(getPath(f"move_horizontal_{self.iconMode}.png")))
        self.moveToTopAction.setIcon(
            QIcon(getPath(f"move_vertical_{self.iconMode}.png")))
        self.restartAction.setIcon(
            QIcon(getPath(f"restart_{self.iconMode}.png")))
        ...
        self.reloadInternetAction.setIcon(
            QIcon(getPath(f"internet_{self.iconMode}.png")))
        self.blacklistAction.setIcon(
            QIcon(getPath(f"blacklistscreen_{self.iconMode}.png")))
...
def drawVerticalLine(canvas: QSize, lineHeight: int, alpha=255):
    ...
    qP = QPainter(pixmap)
    qC = QColor(255, 255, 255, alpha) if isTaskbarDark(
...
def verifyHwndValidity(hwnd):
    ...
    try:
        pHandle = win32api.OpenProcess(
            win32con.PROCESS_QUERY_INFORMATION or win32con.PROCESS_VM_READ, False, pid)
    ...
    if pHandle != 0:
        ...
        if str(pname).lower().endswith("textinputhost.exe") or str(pname).lower().endswith("lockapp.exe") or str(pname).lower().endswith("rtkuwp.exe"):
            ...
            print(
                f"🟡 Blacklisted hwnd {hwnd} under title {win32gui.GetWindowText(hwnd)} (Process name is {str(pname).lower()})")
        else:
            globals.notTextInputHost.append(hwnd)
            print(
                f"🟢 Hwnd {hwnd} under title {win32gui.GetWindowText(hwnd)} was verified as a valid window (Process name is {str(pname).lower()})")
    ...
    try:
        try:
            oldlang = open(os.path.join(os.path.expanduser(
                "~"), ".elevenclock/lang/"+file), "rb").read()
        except FileNotFoundError:
            oldlang = ""
        newlang = urlopen(
            "https://raw.githubusercontent.com/marticliment/ElevenClock/main/elevenclock/lang/"+file)
        if newlang.status == 200:
            ...
            if not os.path.isdir(os.path.join(os.path.expanduser("~"), ".elevenclock/lang/")):
                os.makedirs(os.path.join(
                    os.path.expanduser("~"), ".elevenclock/lang/"))
            if oldlang != langdata:
...
if langName == "":
    ...
    ans = QInputDialog.getItem(w, "Welcome to ElevenClock",
                               "Please select the language you want ElevenClock to use:", languageReference.values(), editable=False)
    w.deleteLater()
    w.close()
    if ans[1] is False:
