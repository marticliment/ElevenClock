...
class SettingsWindow(QMainWindow):
    ...
    def __init__(self):
        ...
        sp.setScrollMetric(
            QScrollerProperties.DragVelocitySmoothingFactor,   1)
        ...
        if lang["locale"] == "zh_TW":
            self.title.setStyleSheet(
                "font-size: 20pt;font-family: \"Microsoft JhengHei UI\";font-weight: 600;")
        elif lang["locale"] == "zh_CN":
            self.title.setStyleSheet(
                "font-size: 20pt;font-family: \"Microsoft YaHei UI\";font-weight: 600;")
        else:
            self.title.setStyleSheet(
                "font-size: 20pt;font-family: \"Segoe UI Variable Text\";font-weight: 700;")
        layout.setSpacing(5)
        ...
        self.updateButton = QSettingsButton(
            _("<b>Update to the latest version!</b>"), _("Install update"))
        self.updateButton.setStyleSheet("")
        self.updateButton.clicked.connect(lambda: KillableThread(
            target=globals.updateIfPossible, args=((True,))).start())
        self.updateButton.setStyleSheet(
            f"QWidget#stBtn{{border-radius: 8px;}}")

        self.updateButton.hide()
        layout.addWidget(self.updateButton)

        self.generalSettingsTitle = QSettingsTitle(_("General Settings:"), getPath(
            f"settings_{self.iconMode}.png"), _("Updates, icon tray, language"))
        layout.addWidget(self.generalSettingsTitle)
        # The non-translated (Language) string is there to let people know what the language option is if you accidentaly change the language
        self.selectedLanguage = QSettingsComboBox(
            _("ElevenClock's language")+" (Language)", _("Change"))
        self.selectedLanguage.setStyleSheet(
        ...
        try:
            self.selectedLanguage.setItems(langListWithPercentage, langListWithPercentage.index(
                langDictWithPercentage[tools.langName]))
        ...
        def changeLang():
            ...
            selectedLang = invertedLangDict[self.selectedLanguage.combobox.currentText(
            )]
        ...
        def restartElevenClockByLangChange():
            subprocess.run(str("start /B \"\" \""+sys.executable) +
                           "\" --settings", shell=True)
            globals.app.quit()

        self.selectedLanguage.restartButton.clicked.connect(
        ...
        self.wizardButton = QSettingsButton(
        ...
        self.enableUpdates = QSettingsCheckBox(
            _("Automatically check for updates"))
        self.enableUpdates.setChecked(
            not self.getSettings("DisableAutoCheckForUpdates"))
        self.enableUpdates.stateChanged.connect(lambda i: self.setSettings(
            "DisableAutoCheckForUpdates", not bool(i), r=False))
        self.generalSettingsTitle.addWidget(self.enableUpdates)
        self.installUpdates = QSettingsCheckBox(
            _("Automatically install available updates"))
        self.installUpdates.setChecked(
            not self.getSettings("DisableAutoInstallUpdates"))
        self.installUpdates.stateChanged.connect(lambda i: self.setSettings(
            "DisableAutoInstallUpdates", not bool(i), r=False))
        ...
        self.silentUpdates = QSettingsCheckBox(
            _("Enable really silent updates"))
        self.silentUpdates.setChecked(self.getSettings("EnableSilentUpdates"))
        self.silentUpdates.stateChanged.connect(
            lambda i: self.setSettings("EnableSilentUpdates", bool(i), r=False))
        self.silentUpdates.setStyleSheet("border-top: 0px solid transparent;")
        self.generalSettingsTitle.addWidget(self.silentUpdates)
        self.enableSystemTray = QSettingsCheckBox(
            _("Show ElevenClock on system tray"))
        self.enableSystemTray.setChecked(
            not self.getSettings("DisableSystemTray"))
        self.enableSystemTray.stateChanged.connect(
            lambda i: self.setSettings("DisableSystemTray", not bool(i)))
        self.generalSettingsTitle.addWidget(self.enableSystemTray)
        self.startupButton = QSettingsButton(
            _("Change startup behaviour"), _("Change"))
        self.startupButton.clicked.connect(
            lambda: os.startfile("ms-settings:startupapps"))
        self.generalSettingsTitle.addWidget(self.startupButton)

        self.clockSettingsTitle = QSettingsTitle(_("Clock Settings:"), getPath(f"clock_{self.iconMode}.png"), _(
            "Fullscreen behaviour, low cpu mode, other miscellaneous preferences"))
        layout.addWidget(self.clockSettingsTitle)
        self.addSecondClock = QSettingsCheckBox(
            _("Show a second clock on the other end of the taskbar"))
        self.addSecondClock.setChecked(self.getSettings("EnableSecondClock"))
        self.addSecondClock.stateChanged.connect(
            lambda i: self.setSettings("EnableSecondClock", bool(i)))
        self.clockSettingsTitle.addWidget(self.addSecondClock)
        self.legacyHideOnFullScreen = QSettingsCheckBox(
            _("Hide the clock in fullscreen mode"))
        self.legacyHideOnFullScreen.setChecked(
            not self.getSettings("DisableHideOnFullScreen"))
        self.legacyHideOnFullScreen.stateChanged.connect(lambda i: (self.setSettings(
            "DisableHideOnFullScreen", not bool(i)), self.updateCheckBoxesStatus()))
        self.clockSettingsTitle.addWidget(self.legacyHideOnFullScreen)
        self.TransparentClockWhenInFullscreen = QSettingsCheckBox(
            _("Force the clock to be transparent if any window shows in fullscreen"))
        self.TransparentClockWhenInFullscreen.setChecked(
            self.getSettings("TransparentClockWhenInFullscreen"))
        self.TransparentClockWhenInFullscreen.stateChanged.connect(
            lambda i: self.setSettings("TransparentClockWhenInFullscreen", bool(i)))
        self.TransparentClockWhenInFullscreen.setStyleSheet(
            "border-top: 0px solid transparent;")
        self.clockSettingsTitle.addWidget(
            self.TransparentClockWhenInFullscreen)
        self.transparentclickEventFS = QSettingsCheckBox(
            _("Ignore mouse clicks when on fullscreen"))
        self.transparentclickEventFS.setChecked(
            self.getSettings("MouseEventTransparentFS"))
        self.transparentclickEventFS.stateChanged.connect(
            lambda i: self.setSettings("MouseEventTransparentFS", bool(i)))
        self.transparentclickEventFS.setStyleSheet(
            "border-top: 0px solid transparent;")
        self.clockSettingsTitle.addWidget(self.transparentclickEventFS)
        self.forceClockToShow = QSettingsCheckBox(
            _("Show the clock when the taskbar is set to hide automatically"))
        self.forceClockToShow.setChecked(
            self.getSettings("DisableHideWithTaskbar"))
        self.forceClockToShow.stateChanged.connect(
            lambda i: self.setSettings("DisableHideWithTaskbar", bool(i)))
        self.clockSettingsTitle.addWidget(self.forceClockToShow)
        self.hideClockWhenClicked = QSettingsCheckBox(
            _("Hide the clock during 10 seconds when clicked"))
        self.hideClockWhenClicked.setChecked(
            self.getSettings("HideClockWhenClicked"))
        self.hideClockWhenClicked.stateChanged.connect(
            lambda i: self.setSettings("HideClockWhenClicked", bool(i)))
        self.clockSettingsTitle.addWidget(self.hideClockWhenClicked)
        self.hideClockWhenHovered = QSettingsCheckBox(
            _("Hide the clock during 5 seconds when hoveredwith the mouse"))
        self.hideClockWhenHovered.setChecked(
            self.getSettings("HideClockWhenHovered"))
        self.hideClockWhenHovered.stateChanged.connect(
            lambda i: self.setSettings("HideClockWhenHovered", bool(i)))
        self.hideClockWhenHovered.setStyleSheet(
            "border-top: 0px solid transparent;")
        self.clockSettingsTitle.addWidget(self.hideClockWhenHovered)
        self.autoreloadclocks = QSettingsCheckBox(
            _("Reload clocks automatically every 5 minutes"))
        self.autoreloadclocks.setChecked(self.getSettings("AutoReloadClocks"))
        self.autoreloadclocks.stateChanged.connect(
            lambda i: self.setSettings("AutoReloadClocks", bool(i)))
        self.clockSettingsTitle.addWidget(self.autoreloadclocks)
        self.enableLowCpuMode = QSettingsCheckBoxWithWarning(_("Enable low-cpu mode"), _(
            "You might lose functionalities, like the notification counter or the dynamic background"))
        self.enableLowCpuMode.setStyleSheet(
            f"QWidget#stChkBg{{border-bottom-left-radius: 8px;border-bottom-right-radius: 8px;border-bottom: 1px;}}")
        self.enableLowCpuMode.setChecked(self.getSettings("EnableLowCpuMode"))
        self.enableLowCpuMode.stateChanged.connect(
            lambda i: self.setSettings("EnableLowCpuMode", bool(i)))
        self.clockSettingsTitle.addWidget(self.enableLowCpuMode)

        self.clockFeaturesTitle = QSettingsTitle(_("Clock features:"), getPath(
            f"plugin_{self.iconMode}.png"), _("Notification badge, clicked action, show desktop button, etc."))
        layout.addWidget(self.clockFeaturesTitle)
        self.disableNotificationBadge = QSettingsCheckBox(
            _("Disable the notification badge"))
        self.disableNotificationBadge.setChecked(
            self.getSettings("DisableNotifications"))
        self.disableNotificationBadge.stateChanged.connect(
            lambda i: self.setSettings("DisableNotifications", bool(i)))
        self.clockFeaturesTitle.addWidget(self.disableNotificationBadge)
        self.disableTooltip = QSettingsCheckBox(
            _("Disable the tooltip shown when the clock is hovered"))
        self.disableTooltip.setChecked(self.getSettings("DisableToolTip"))
        self.disableTooltip.stateChanged.connect(
            lambda i: self.setSettings("DisableToolTip", bool(i)))
        self.clockFeaturesTitle.addWidget(self.disableTooltip)
        self.showDesktopButton = QSettingsCheckBox(
            _("Add the \"Show Desktop\" button on the left corner of every clock"))
        self.showDesktopButton.setChecked(
            self.getSettings("ShowDesktopButton"))
        self.showDesktopButton.stateChanged.connect(
            lambda i: self.setSettings("ShowDesktopButton", bool(i)))
        ...
        self.customClockAction = QSettingsSizeBoxComboBox(
            _("Change the action done when the clock is clicked"))
        clickActions = {
        ...
        self.customClockAction.setChecked(
            self.getSettings("CustomClockClickAction"))
        # region Extra option for the "toggle_setting_click" action (customClockClickToggleAction)
        self.customClockClickToggleAction = QSettingsComboBox(
            _("Choose the setting to toggle when clock is clicked"))
        self.customClockClickToggleAction.setStyleSheet(
            "#stBtn{border-top: 0px solid transparent;border-radius: 0px;}")
        self.customClockClickToggleAction.loadItems(toggleactions.keys())
        self.customClockClickToggleAction.setEnabled(True)
        try:
            self.customClockClickToggleAction.combobox.setCurrentIndex(list(
                toggleactions.values()).index(self.getSettingsValue("ClockClickToggleSetting")))
        except ValueError:
            pass
        self.customClockClickToggleAction.valueChanged.connect(
            lambda v: self.setSettingsValue("ClockClickToggleSetting", toggleactions[v]))
        # endregion

        def customClickActionValueChanged(v):
            self.setSettingsValue("CustomClockClickAction", clickActions[v])
            self.customClockClickToggleAction.setEnabled(
                clickActions[v] == "toggle_setting_click")
        try:
            self.customClockAction.combobox.setCurrentIndex(list(
                clickActions.values()).index(self.getSettingsValue("CustomClockClickAction")))
        except ValueError:
            pass
        self.customClockAction.stateChanged.connect(
            lambda i: self.setSettings("CustomClockClickAction", bool(i)))
        self.customClockAction.valueChanged.connect(
            customClickActionValueChanged)
        ...
        self.customDoubleClickAction = QSettingsSizeBoxComboBox(
            _("Change the action done when the clock is double-clicked"))
        self.customDoubleClickAction.setStyleSheet(
            "border-top: 0px solid transparent;")
        doubleActions = {
            _("Show calendar"): "Win+N",
            _("Copy date/time"): "copy_datetime",
        ...
        self.customDoubleClickAction.setChecked(
            self.getSettings("CustomClockDoubleClickAction"))
        # region Extra option for the "toggle_setting_dblclick" action (customDoubleClickToggleAction)
        self.customDoubleClickToggleAction = QSettingsComboBox(
            _("Choose the setting to toggle when double-clicked"))
        self.customDoubleClickToggleAction.setStyleSheet(
            "#stBtn{border-top: 0px solid transparent;border-radius: 0px;}")
        self.customDoubleClickToggleAction.loadItems(toggleactions.keys())
        self.customDoubleClickToggleAction.setEnabled(True)
        try:
            self.customDoubleClickToggleAction.combobox.setCurrentIndex(list(
                toggleactions.values()).index(self.getSettingsValue("DoubleClickToggleSetting")))
        except ValueError:
            pass
        self.customDoubleClickToggleAction.valueChanged.connect(
            lambda v: self.setSettingsValue("DoubleClickToggleSetting", toggleactions[v]))
        # endregion

        def customDoubleClickActionValueChanged(v):
            self.setSettingsValue(
                "CustomClockDoubleClickAction", doubleActions[v])
            self.customDoubleClickToggleAction.setEnabled(
                doubleActions[v] == "toggle_setting_dblclick")
        self.customDoubleClickAction.stateChanged.connect(
            lambda i: self.setSettings("CustomClockDoubleClickAction", bool(i)))
        self.customDoubleClickAction.valueChanged.connect(
            customDoubleClickActionValueChanged)
        try:
            self.customDoubleClickAction.combobox.setCurrentIndex(list(
                doubleActions.values()).index(self.getSettingsValue("CustomClockDoubleClickAction")))
        except ValueError:
            pass
        ...
        self.customMiddleClickAction = QSettingsSizeBoxComboBox(
            _("Change the action done when the clock is middle-clicked"))
        self.customMiddleClickAction.setStyleSheet(
            "border-top: 0px solid transparent;")
        middleActions = {
            _("Show calendar"): "Win+N",
            _("Copy date/time"): "copy_datetime",
        ...
        self.customMiddleClickAction.setChecked(
            self.getSettings("CustomClockMiddleClickAction"))
        # region Extra option for the "toggle_setting_mdlclick" action (customMiddleClickToggleAction)
        self.customMiddleClickToggleAction = QSettingsComboBox(
            _("Choose the setting to toggle when middle-clicked"))
        self.customMiddleClickToggleAction.setStyleSheet(
            "border-top: 0px solid transparent;")
        self.customMiddleClickToggleAction.loadItems(toggleactions.keys())
        self.customMiddleClickToggleAction.setEnabled(True)
        try:
            self.customMiddleClickToggleAction.combobox.setCurrentIndex(list(
                toggleactions.values()).index(self.getSettingsValue("MiddleClickToggleSetting")))
        except ValueError:
            pass
        self.customMiddleClickToggleAction.valueChanged.connect(
            lambda v: self.setSettingsValue("MiddleClickToggleSetting", toggleactions[v]))
        # endregion

        def customMiddleClickActionValueChanged(v):
            self.setSettingsValue(
                "CustomClockMiddleClickAction", middleActions[v])
            self.customMiddleClickToggleAction.setEnabled(
                middleActions[v] == "toggle_setting_mdlclick")
        try:
            self.customMiddleClickAction.combobox.setCurrentIndex(list(
                middleActions.values()).index(self.getSettingsValue("CustomClockMiddleClickAction")))
        except ValueError:
            pass
        self.customMiddleClickAction.stateChanged.connect(
            lambda i: self.setSettings("CustomClockMiddleClickAction", bool(i)))
        self.customMiddleClickAction.valueChanged.connect(
            customMiddleClickActionValueChanged)
        self.clockFeaturesTitle.addWidget(self.customMiddleClickAction)
        # There is a bug where middle-clicking causes infinte loop that runs the action multiple times, so I'll disable it now
        self.clockFeaturesTitle.addWidget(self.customMiddleClickToggleAction)
        # endregion

        self.clockPosTitle = QSettingsTitle(_("Clock position and size:"), getPath(
            f"size_{self.iconMode}.png"), _("Clock size preferences, position offset, clock at the left, etc."))
        layout.addWidget(self.clockPosTitle)
        self.clockAtLeft = QSettingsCheckBox(
            _("Show the clock at the left of the screen"))
        self.clockAtLeft.setChecked(self.getSettings("ClockOnTheLeft"))
        self.clockAtLeft.stateChanged.connect(
            lambda i: self.setSettings("ClockOnTheLeft", bool(i)))
        self.clockPosTitle.addWidget(self.clockAtLeft)
        self.clockAtBottom = QSettingsCheckBox(
            _("Force the clock to be at the bottom of the screen"))
        self.clockAtBottom.setChecked(self.getSettings("ForceOnBottom"))
        self.clockAtBottom.stateChanged.connect(
            lambda i: self.setSettings("ForceOnBottom", bool(i)))
        self.clockPosTitle.addWidget(self.clockAtBottom)
        self.clockAtTop = QSettingsCheckBox(
            _("Force the clock to be at the top of the screen"))
        self.clockAtTop.setChecked(self.getSettings("ForceOnTop"))
        self.clockAtTop.stateChanged.connect(
            lambda i: self.setSettings("ForceOnTop", bool(i)))
        self.clockPosTitle.addWidget(self.clockAtTop)
        self.clockAtTop.setStyleSheet("border-top: 0px solid transparent;")
        self.PinClockToDesktop = QSettingsCheckBox(
            _("Pin the clock to the desktop"))
        self.PinClockToDesktop.setChecked(
            self.getSettings("PinClockToTheDesktop"))
        self.PinClockToDesktop.stateChanged.connect(
            lambda i: self.setSettings("PinClockToTheDesktop", bool(i)))
        self.clockPosTitle.addWidget(self.PinClockToDesktop)
        self.clockFixedHeight = QSettingsSliderWithCheckBox(
            _("Change the height of the clock"), self, 20, 105, 48)
        self.clockFixedHeight.setChecked(self.getSettings("ClockFixedHeight"))
        if self.clockFixedHeight.isChecked():
            try:
                self.clockFixedHeight.slider.setValue(
                    int(self.getSettingsValue("ClockFixedHeight")))
            except ValueError:
                print("🟠 Unable to parse int from ClockFixedHeight settings value")
        self.clockFixedHeight.stateChanged.connect(
            lambda v: self.setSettings("ClockFixedHeight", bool(v)))
        self.clockFixedHeight.valueChanged.connect(
            lambda v: self.setSettingsValue("ClockFixedHeight", str(v)))
        self.clockPosTitle.addWidget(self.clockFixedHeight)
        self.ClockFixedWidth = QSettingsSliderWithCheckBox(
            _("Change the width of the clock"), self, 30, 200, 48)
        self.ClockFixedWidth.setStyleSheet(
            "border-top: 0px solid transparent;")
        self.ClockFixedWidth.setChecked(self.getSettings("ClockFixedWidth"))
        if self.ClockFixedWidth.isChecked():
            try:
                self.ClockFixedWidth.slider.setValue(
                    int(self.getSettingsValue("ClockFixedWidth")))
            except ValueError:
                print("🟠 Unable to parse int from ClockFixedWidth settings value")
        self.ClockFixedWidth.stateChanged.connect(
            lambda v: self.setSettings("ClockFixedWidth", bool(v)))
        self.ClockFixedWidth.valueChanged.connect(
            lambda v: self.setSettingsValue("ClockFixedWidth", str(v)))
        self.clockPosTitle.addWidget(self.ClockFixedWidth)

        self.clockXOffset = QSettingsSliderWithCheckBox(
            _("Adjust horizontal clock position"), self, -200, 200, 0)
        self.clockXOffset.setChecked(self.getSettings("ClockXOffset"))
        if self.clockXOffset.isChecked():
            try:
                self.clockXOffset.slider.setValue(
                    int(self.getSettingsValue("ClockXOffset")))
            except ValueError:
                print("🟠 Unable to parse int from ClockXOffset settings value")
        self.clockXOffset.stateChanged.connect(
            lambda v: self.setSettings("ClockXOffset", bool(v)))
        self.clockXOffset.valueChanged.connect(
            lambda v: self.setSettingsValue("ClockXOffset", str(v)))
        self.clockPosTitle.addWidget(self.clockXOffset)

        self.clockYOffset = QSettingsSliderWithCheckBox(
            _("Adjust vertical clock position"), self, -200, 200, 0)
        ...
        if self.clockYOffset.isChecked():
            try:
                self.clockYOffset.slider.setValue(
                    int(self.getSettingsValue("ClockYOffset")))
            except ValueError:
                print("🟠 Unable to parse int from clockYOffset settings value")
        self.clockYOffset.stateChanged.connect(
            lambda v: self.setSettings("ClockYOffset", bool(v)))
        self.clockYOffset.valueChanged.connect(
            lambda v: self.setSettingsValue("ClockYOffset", str(v)))
    ...
    {_("Now you should see the clock everywhere")}""")
            ...
            msg.setDefaultButtonRole(
                QDialogButtonBox.ButtonRole.ApplyRole, self.styleSheet())
            msg.show()

        self.unBlackListButton = QSettingsButton(
            _("Reset monitor blacklisting status"), _("Reset"))
        self.unBlackListButton.clicked.connect(unblacklist)
        self.clockPosTitle.addWidget(self.unBlackListButton)

        self.clockAppearanceTitle = QSettingsTitle(_("Clock Appearance:"), getPath(
            f"appearance_{self.iconMode}.png"), _("Clock's font, font size, font color and background, text alignment"))
        ...
        self.fontPrefs.stateChanged.connect(
            lambda i: self.setSettings("UseCustomFont", bool(i)))
        self.fontPrefs.valueChanged.connect(
            lambda v: self.setSettingsValue("UseCustomFont", v))
        ...
        self.fontSize.stateChanged.connect(
            lambda i: self.setSettings("UseCustomFontSize", bool(i)))

        def setCustomFont(v):
            self.setSettingsValue("UseCustomFontSize", v,
                                  r=not (self.fontPrefs.isChecked()))
            if self.fontPrefs.isChecked():
        ...
        self.fontColor = QSettingsCheckboxColorDialog(
            _("Use a custom font color"))
        ...
        if self.fontColor.isChecked():
            self.fontColor.button.setStyleSheet(
                f"color: rgb({self.getSettingsValue('UseCustomFontColor')})")
        self.fontColor.stateChanged.connect(lambda i: (
            self.setSettings("UseCustomFontColor", bool(i))))
        self.fontColor.valueChanged.connect(
            lambda v: self.setSettingsValue("UseCustomFontColor", v))
        self.clockAppearanceTitle.addWidget(self.fontColor)

        self.lineHeight = QSettingsSizeBoxComboBox(
            _("Use a custom line height"))
        ...
        self.lineHeight.stateChanged.connect(
            lambda i: self.setSettings("CustomLineHeight", bool(i)))
        self.lineHeight.valueChanged.connect(
            lambda v: self.setSettingsValue("CustomLineHeight", v))
        self.centerText = QSettingsCheckBox(
            _("Align the clock text to the center"))
        self.centerText.setChecked(self.getSettings("CenterAlignment"))
        self.centerText.stateChanged.connect(
            lambda i: self.setSettings("CenterAlignment", bool(i)))
        self.clockAppearanceTitle.addWidget(self.centerText)
        self.clockAppearanceTitle.addWidget(self.lineHeight)
        self.backgroundcolor = QSettingsBgBoxColorDialog(
            _("Use a custom background color"))
        self.backgroundcolor.setChecked(self.getSettings("UseCustomBgColor"))
        self.backgroundcolor.colorDialog.setOption(
            QColorDialog.ShowAlphaChannel, True)
        if self.backgroundcolor.isChecked():
            self.backgroundcolor.button.setStyleSheet(
                f"background-color: rgba({self.getSettingsValue('UseCustomBgColor')})")
        self.backgroundcolor.stateChanged.connect(
            lambda i: self.setSettings("UseCustomBgColor", bool(i)))
        self.backgroundcolor.valueChanged.connect(
            lambda v: self.setSettingsValue("UseCustomBgColor", v))
        self.clockAppearanceTitle.addWidget(self.backgroundcolor)
        self.disableClocckBlurryBackgound = QSettingsCheckBox(
            _("Disable clock blurry texture"))
        self.disableClocckBlurryBackgound.setStyleSheet(
            "border-top: 0px solid transparent;")
        self.disableClocckBlurryBackgound.setChecked(
            self.getSettings("DisableBlurryTexture"))
        self.disableClocckBlurryBackgound.stateChanged.connect(
            lambda i: self.setSettings("DisableBlurryTexture", bool(i)))
        self.disableClocckBlurryBackgound.setStyleSheet(
            f"QWidget#stChkBg{{border-bottom-left-radius: 8px;border-bottom-right-radius: 8px;border-bottom: 1px;}}")
        self.clockAppearanceTitle.addWidget(self.disableClocckBlurryBackgound)

        self.dateTimeTitle = QSettingsTitle(_("Date & Time Settings:"), getPath(f"datetime_{self.iconMode}.png"), _(
        ...
        """
        self.customDateTimeFormat = QSettingsLineEditCheckBox(
            _("Set a custom date and time format")+" "+_("(for advanced users only)"))
        self.customDateTimeFormat.edit.setPlainText(
            self.getSettingsValue("CustomClockStrings"))
        self.customDateTimeFormat.setLabelText(rulesText)
        self.customDateTimeFormat.stateChanged.connect(lambda i: (self.setSettings(
            "CustomClockStringsDisabled", not bool(i)), self.dateTimeTitle.resizeEvent()))
        self.customDateTimeFormat.valueChanged.connect(
            lambda v: self.setSettingsValue("CustomClockStrings", v))
        self.dateTimeTitle.addWidget(self.customDateTimeFormat)
        self.showTime = QSettingsCheckBox(_("Show time on the clock"))
        self.showTime.stateChanged.connect(
            lambda i: self.setSettings("DisableTime", not bool(i)))
        self.dateTimeTitle.addWidget(self.showTime)
        self.showSeconds = QSettingsCheckBox(_("Show seconds on the clock"))
        self.showSeconds.stateChanged.connect(
            lambda i: self.setSettings("EnableSeconds", bool(i)))
        self.dateTimeTitle.addWidget(self.showSeconds)
        self.showDate = QSettingsCheckBox(_("Show date on the clock"))
        self.showDate.stateChanged.connect(
            lambda i: self.setSettings("DisableDate", not bool(i)))
        self.dateTimeTitle.addWidget(self.showDate)
        self.showWeekCount = QSettingsCheckBox(
            _("Show week number on the clock"))
        self.showWeekCount.stateChanged.connect(
            lambda i: self.setSettings("EnableWeekNumber", bool(i)))
        self.dateTimeTitle.addWidget(self.showWeekCount)
        self.showWeekday = QSettingsCheckBox(_("Show weekday on the clock"))
        self.showWeekday.stateChanged.connect(
            lambda i: self.setSettings("EnableWeekDay", bool(i)))
        self.dateTimeTitle.addWidget(self.showWeekday)
        self.RegionButton = QSettingsButton(
            _("Change date and time format (Regional settings)"), _("Regional settings"))
        self.RegionButton.clicked.connect(lambda: os.startfile("intl.cpl"))
        self.dateTimeTitle.addWidget(self.RegionButton)

        self.internetTimeTitle = QSettingsTitle(_("Internet date and time"), getPath(
            f"internet_{self.iconMode}.png"), _("Select internet time provider, change sync frequency"))
        layout.addWidget(self.internetTimeTitle)

        self.internetTime = QSettingsCheckBox(
            _("Enable internet time sync"), None)
        self.internetTime.setChecked(self.getSettings("EnableInternetTime"))
        self.internetTime.stateChanged.connect(
            lambda e: self.setSettings("EnableInternetTime", e))
        self.internetTimeTitle.addWidget(self.internetTime)

        self.internetTimeURL = QSettingsCheckBoxTextBox(
            _("Set a custom network time provider"), None, f"<a style='color:rgb({getColors()[2 if isWindowDark() else 4]})' href=\"https://www.marticliment.com/redirect?ECNetworkTime\">{_('Help')}</a>")
        self.internetTimeURL.setPlaceholderText(
            _("Paste a URL from the world clock api or equivalent"))
        ...
        self.internetTimeURL.stateChanged.connect(
            lambda e: self.setSettings("AtomicClockURL", e))
        self.internetTimeURL.valueChanged.connect(lambda v: (self.setSettingsValue(
            "AtomicClockURL", v, r=False), self.setSettings("ReloadInternetTime", True)))
        ...
        try:
            self.internetSyncTime.combobox.setCurrentIndex(
                list(actions.values()).index(self.getSettingsValue("AtomicClockSyncInterval")))
        except ValueError:
            pass
        self.internetSyncTime.valueChanged.connect(lambda v: (self.setSettingsValue(
            "AtomicClockSyncInterval", actions[v]), self.setSettings("ReloadInternetTime", True)))
        self.internetTimeTitle.addWidget(self.internetSyncTime)

        self.toolTipAppearanceTitle = QSettingsTitle(_("Tooltip Appearance:"), getPath(
            f"tooltip_{self.iconMode}.png"), _("Tooltip's font, font size, font color and background"))
        layout.addWidget(self.toolTipAppearanceTitle)
        self.toolTipFontPrefs = QSettingsNewFontBoxComboBox(
            _("Use a custom font"))
        self.toolTipFontPrefs.setChecked(
            self.getSettings("TooltipUseCustomFont"))
        if self.toolTipFontPrefs.isChecked():
            ...
            if customFont:
                ...
                self.fontPrefs.fontPicker.setCurrentFont(f)
                self.fontPrefs.button.setFont(f)
        self.toolTipFontPrefs.stateChanged.connect(
            lambda i: self.setSettings("TooltipUseCustomFont", bool(i)))
        self.toolTipFontPrefs.valueChanged.connect(
            lambda v: self.setSettingsValue("TooltipUseCustomFont", v))
        self.toolTipAppearanceTitle.addWidget(self.toolTipFontPrefs)

        self.toolTipFontSize = QSettingsSizeBoxComboBox(
            _("Use a custom font size"))
        self.toolTipFontSize.setStyleSheet(
            "border-top: 0px solid transparent;")
        self.toolTipFontSize.setChecked(
            self.getSettings("TooltipUseCustomFontSize"))
        ...
        self.toolTipFontSize.stateChanged.connect(
            lambda i: self.setSettings("TooltipUseCustomFontSize", bool(i)))
        self.toolTipFontSize.valueChanged.connect(
            lambda v: self.setSettingsValue("TooltipUseCustomFontSize", v))
        self.toolTipAppearanceTitle.addWidget(self.toolTipFontSize)

        self.toolTipFontColor = QSettingsCheckboxColorDialog(
            _("Use a custom font color"))
        self.toolTipFontColor.setStyleSheet(
            "border-top: 0px solid transparent;")
        self.toolTipFontColor.setChecked(
            self.getSettings("TooltipUseCustomFontColor"))
        if self.toolTipFontColor.isChecked():
            self.toolTipFontColor.button.setStyleSheet(
                f"color: rgb({self.getSettingsValue('TooltipUseCustomFontColor')})")
        self.toolTipFontColor.stateChanged.connect(
            lambda i: self.setSettings("TooltipUseCustomFontColor", bool(i)))
        self.toolTipFontColor.valueChanged.connect(
            lambda v: self.setSettingsValue("TooltipUseCustomFontColor", v))
        self.toolTipAppearanceTitle.addWidget(self.toolTipFontColor)
        self.disableBlurryBackground = QSettingsCheckBox(
            _("Disable tooltip's blurry background"))
        self.disableBlurryBackground.setChecked(
            self.getSettings("TooltipDisableTaskbarBackgroundColor"))
        self.disableBlurryBackground.stateChanged.connect(
            lambda i: self.setSettings("TooltipDisableTaskbarBackgroundColor", bool(i)))
        self.toolTipAppearanceTitle.addWidget(self.disableBlurryBackground)
        self.toolTipBackgroundColor = QSettingsBgBoxColorDialog(
            _("Use a custom background color"))
        self.toolTipBackgroundColor.setChecked(
            self.getSettings("TooltipUseCustomBgColor"))
        self.toolTipBackgroundColor.colorDialog.setOption(
            QColorDialog.ShowAlphaChannel, True)
        if self.toolTipBackgroundColor.isChecked():
            self.toolTipBackgroundColor.button.setStyleSheet(
                f"background-color: rgba({self.getSettingsValue('TooltipUseCustomBgColor')})")
        self.toolTipBackgroundColor.stateChanged.connect(
            lambda i: self.setSettings("TooltipUseCustomBgColor", bool(i)))
        self.toolTipBackgroundColor.valueChanged.connect(
            lambda v: self.setSettingsValue("TooltipUseCustomBgColor", v))
        self.toolTipBackgroundColor.setStyleSheet(
            f"QWidget#stChkBg{{border-top: 0px solid transparent;border-bottom-left-radius: 8px;border-bottom-right-radius: 8px;border-bottom: 1px;}}")
        self.toolTipAppearanceTitle.addWidget(self.toolTipBackgroundColor)

        self.experimentalTitle = QSettingsTitle(_("Fixes and other experimental features: (Use ONLY if something is not working)"), getPath(
            f"experiment_{self.iconMode}.png"), _("Testing features and error-fixing tools"))
        layout.addWidget(self.experimentalTitle)
        self.bypassCNAMECheck = QSettingsCheckBox(
            _("Bypass update provider authenticity check (NOT RECOMMENDED, AT YOUR OWN RISK)"))
        self.bypassCNAMECheck.setChecked(
            self.getSettings("BypassDomainAuthCheck"))
        self.bypassCNAMECheck.stateChanged.connect(
            lambda i: self.setSettings("BypassDomainAuthCheck", bool(i), r=False))
        self.experimentalTitle.addWidget(self.bypassCNAMECheck)
        self.disableZeroNotifications = QSettingsCheckBox(
            _("Enable the zero on the notification counter (Windows 11 build 22631 or higher)"))
        self.disableZeroNotifications.setChecked(
            not self.getSettings("DisableNewZeroNotificationCounter"))
        self.disableZeroNotifications.stateChanged.connect(
            lambda i: self.setSettings("DisableNewZeroNotificationCounter", not bool(i)))
        self.experimentalTitle.addWidget(self.disableZeroNotifications)
        self.fixDash = QSettingsCheckBox(
            _("Reload clocks right after exiting from sleep"))
        self.fixDash.setChecked(self.getSettings("PreventSleepFailure"))
        self.fixDash.stateChanged.connect(
            lambda i: self.setSettings("PreventSleepFailure", bool(i)))
        self.experimentalTitle.addWidget(self.fixDash)
        self.disableLang = QSettingsCheckBox(
            _("Do NOT update the language files dynamically"))
        self.disableLang.setChecked(self.getSettings("DisableLangAutoUpdater"))
        self.disableLang.stateChanged.connect(
            lambda i: self.setSettings("DisableLangAutoUpdater", bool(i)))
        self.experimentalTitle.addWidget(self.disableLang)
        self.longerDoubleClickPeriod = QSettingsCheckBox(
            _("Increase the length of the double-click period"))
        self.longerDoubleClickPeriod.setChecked(
            self.getSettings("DoubleClickLongerPeriod"))
        self.longerDoubleClickPeriod.stateChanged.connect(
            lambda i: self.setSettings("DoubleClickLongerPeriod", bool(i)))
        self.experimentalTitle.addWidget(self.longerDoubleClickPeriod)
        self.disableClockCover = QSettingsCheckBox(
            _("Disable hiding the default windows clock"))
        self.disableClockCover.setChecked(
            self.getSettings("DisableSystemClockCover"))
        self.disableClockCover.stateChanged.connect(
            lambda i: self.setSettings("DisableSystemClockCover", bool(i)))
        self.disableClockCover.setStyleSheet(
            f"QWidget#stChkBg{{border-bottom-left-radius: 8px;border-bottom-right-radius: 8px;border-bottom: 1px;}}")
        self.experimentalTitle.addWidget(self.disableClockCover)

        self.languageSettingsTitle = QSettingsTitle(_("About the language pack:"), getPath(
            f"lang_{self.iconMode}.png"), _("Language pack author(s), help translating ElevenClock"))
        layout.addWidget(self.languageSettingsTitle)

        translatorsHTMLList = _(
            "ElevenClock has not been machine translated. The following users have been in charge of the translations:")
        ...
        translators.setStyleSheet(
            "QWidget#stBtn{border-bottom-left-radius: 0;border-bottom-right-radius: 0;border-bottom: 0;padding: 10px;padding-left: 55px;padding-right: 55px;}")
        self.languageSettingsTitle.addWidget(translators)

        self.openTranslateButton = QSettingsButton(
            _("Translate ElevenClock to your language"), _("Get started"))
        self.openTranslateButton.clicked.connect(lambda: os.startfile(
            "https://github.com/marticliment/ElevenClock/wiki/#translating-elevenclock"))
        ...
        def thirdPartyLicenses():
            ...
            msg.setObjectName("QMessageBox")
            msg.setTitle(
                _("Third Party Open-Source Software in Elevenclock {0} (And their licenses)").format(versionName))
            colors = getColors()
            msg.setText(f"""
            ...
            msg.addButton(
                _("Ok"), QDialogButtonBox.ButtonRole.ApplyRole, lambda: msg.close())
            msg.addButton(_("More Info"), QDialogButtonBox.ButtonRole.ResetRole, lambda: os.startfile(
                "https://github.com/marticliment/ElevenClock/wiki#third-party-libraries"))

            def closeAndQt():
                msg.close()
                QMessageBox.aboutQt(self, "ElevenClock - "+_("About Qt"))
            msg.addButton(
                _("About Qt"), QDialogButtonBox.ButtonRole.ResetRole, lambda: closeAndQt())
            msg.setDefaultButtonRole(
                QDialogButtonBox.ButtonRole.ApplyRole, self.styleSheet())
            msg.show()
        ...
        def exportSettings():
            ...
            try:
                ...
                fileName = QFileDialog.getSaveFileName(self, _("Export settings to a local file"), os.path.expanduser(
                    "~"), "ElevenClock Settings File (*.esf);;All Files (*.*)")
                if fileName[0] != "":
                ...
                if fileName:
                    ...
                    for element in rawstr.split("|~|"):
                        ...
                        if len(pairValue) == 2:
                            ...
                            if pairValue[1] != "":
                                setSettingsValue(
                                    pairValue[0], pairValue[1], r=False)
        ...
        self.aboutTitle = QSettingsTitle(_("About ElevenClock version {0}:").format(versionName), getPath(
            f"about_{self.iconMode}.png"), _("Info, report a bug, submit a feature request, donate, about"))
        layout.addWidget(self.aboutTitle)
        self.WebPageButton = QSettingsButton(
            _("View ElevenClock's homepage"), _("Open"))
        self.WebPageButton.clicked.connect(
            lambda: os.startfile("https://marticliment.com/elevenclock/"))
        self.WebPageButton.setStyleSheet(
            "QWidget#stBtn{border-bottom-left-radius: 0;border-bottom-right-radius: 0;border-bottom: 0;}")
        ...
        self.ThirdParty.setStyleSheet(
            "QWidget#stBtn{border-top: 0px solid transparent;border-bottom-left-radius: 0;border-bottom-right-radius: 0;border-bottom: 0;}")
        self.aboutTitle.addWidget(self.ThirdParty)
        self.IssueButton = QSettingsButton(
            _("Report an issue/request a feature"), _("Report"))
        self.IssueButton.clicked.connect(lambda: os.startfile(
            "https://github.com/marticliment/ElevenClock/issues/new/choose"))
        self.IssueButton.setStyleSheet(
            "QWidget#stBtn{border-bottom-left-radius: 0;border-bottom-right-radius: 0;border-bottom: 0;}")
        self.aboutTitle.addWidget(self.IssueButton)
        self.CofeeButton = QSettingsButton(
            _("Support the dev: Give me a coffee☕"), _("Open page"))
        self.CofeeButton.clicked.connect(
            lambda: os.startfile("https://ko-fi.com/martinet101"))
        self.CofeeButton.setStyleSheet(
            "QWidget#stBtn{border-bottom-left-radius: 0;border-bottom-right-radius: 0;border-bottom: 0;}")
        self.aboutTitle.addWidget(self.CofeeButton)
        self.importSettings = QSettingsButton(
        ...
        self.exportSettings = QSettingsButton(
        ...
        self.resetButton = QSettingsButton(
            _("Reset ElevenClock preferences to defaults"), _("Reset"))
        ...
        contributorsHTMLList = _(
            "ElevenClock wouldn't have been possible with the help of our dear contributors:")
        ...
        contributors.setStyleSheet(
            "QWidget#stBtn{border-bottom-left-radius: 0;border-bottom-right-radius: 0;border-bottom: 0;padding: 20px;padding-left: 55px;padding-right: 55px;}")
        ...
        self.debbuggingTitle = QSettingsTitle(_("Debbugging information:"), getPath(
            f"bug_{self.iconMode}.png"), _("Log, debugging information"))
        layout.addWidget(self.debbuggingTitle)
        self.helpButton = QSettingsButton(
            _("Open online help to troubleshoot problems"), _("Open"))
        self.helpButton.setStyleSheet(
            "QWidget#stBtn{border-bottom-left-radius: 0;border-bottom-right-radius: 0;border-bottom: 0;}")
        self.helpButton.clicked.connect(lambda: os.startfile(
            "https://github.com/marticliment/ElevenClock/wiki#Troubleshooting"))
        self.debbuggingTitle.addWidget(self.helpButton)
        self.logButton = QSettingsButton(
            _("Open ElevenClock's log"), _("Open"))
        ...
        if isWindowDark():
            self.notFoundLabel.setStyleSheet(
                f"padding-top: 30px;font-size: 16pt; font-weight: bold; color: rgba(255, 255, 255, 50%)")
        else:
            self.notFoundLabel.setStyleSheet(
                f"padding-top: 30px;font-size: 16pt; font-weight: bold; color: rgba(0, 0, 0, 50%)")
        ...
        self.scrollArea.setStyleSheet(
            f"QScrollArea{{border-bottom-left-radius: 8px;border-bottom-right-radius: 8px;}}")
    ...
    def updateCheckBoxesStatus(self):
        ...
        if not self.enableUpdates.isChecked():  # Check if check for updates enabled
            for checkbox in [self.installUpdates, self.silentUpdates, self.bypassCNAMECheck]:
                checkbox.setToolTip(
                    _("<b>{0}</b> needs to be enabled to change this setting").format(_("Automatically check for updates")))
            ...
            if not self.installUpdates.isChecked():  # Check if install updates enabled
                for checkbox in [self.silentUpdates, self.bypassCNAMECheck]:
                    checkbox.setToolTip(_("<b>{0}</b> needs to be enabled to change this setting").format(
                        _("Automatically install available updates")))
                    checkbox.setEnabled(False)
            else:
                for checkbox in [self.silentUpdates, self.bypassCNAMECheck]:
        ...
        if not self.customDateTimeFormat.isChecked():
            ...
            if not self.showTime.isChecked():  # Check if time is shown
                self.showSeconds.setToolTip(
                    _("<b>{0}</b> needs to be enabled to change this setting").format(_("Show time on the clock")))
            ...
            if not self.showDate.isChecked():  # Check if date is shown
                self.showWeekCount.setToolTip(
                    _("<b>{0}</b> needs to be enabled to change this setting").format(_("Show date on the clock")))
        ...
        if not self.legacyHideOnFullScreen.isChecked():
            ...
            self.TransparentClockWhenInFullscreen.setToolTip(
                _("<b>{0}</b> needs to be enabled to change this setting").format(_("Hide the clock in fullscreen mode")))

        if self.enableLowCpuMode.isChecked():
            self.disableNotificationBadge.setToolTip(
                _("<b>{0}</b> needs to be disabled to change this setting").format(_("Enable low-cpu mode")))
        ...
        if self.internetTime.isChecked():
            ...
            self.internetSyncTime.setToolTip(
                _("<b>{0}</b> needs to be enabled to change this setting").format(_("Sync time with the internet")))
            self.internetTimeURL.setEnabled(False)
            self.internetTimeURL.setToolTip(
                _("<b>{0}</b> needs to be enabled to change this setting").format(_("Sync time with the internet")))

        # Custom clock click action
        isExtraOptionEnabled = self.customClockAction.isChecked(
        ) and self.getSettingsValue("CustomClockClickAction") == "toggle_setting_click"
        self.customClockClickToggleAction.setEnabled(isExtraOptionEnabled)
        isExtraOptionEnabled = self.customDoubleClickAction.isChecked() and self.getSettingsValue(
            "CustomClockDoubleClickAction") == "toggle_setting_dblclick"
        self.customDoubleClickToggleAction.setEnabled(isExtraOptionEnabled)
        isExtraOptionEnabled = self.customMiddleClickAction.isChecked() and self.getSettingsValue(
            "CustomClockMiddleClickAction") == "toggle_setting_mdlclick"
        ...
        self.customDateTimeFormat.setChecked(not self.getSettings(
            "CustomClockStringsDisabled") and self.getSettingsValue("CustomClockStrings") != False)
    ...
    def applyStyleSheet(self):
        ...
        if isWindowDark():
            ...
            if self.lastTheme != 0:
                ...
                self.dateTimeTitle.setIcon(
                    getPath(f"datetime_{self.iconMode}.png"))
                self.clockSettingsTitle.setIcon(
                    getPath(f"clock_{self.iconMode}.png"))
                self.clockFeaturesTitle.setIcon(
                    getPath(f"plugin_{self.iconMode}.png"))
                self.languageSettingsTitle.setIcon(
                    getPath(f"lang_{self.iconMode}.png"))
                self.generalSettingsTitle.setIcon(
                    getPath(f"settings_{self.iconMode}.png"))
                self.experimentalTitle.setIcon(
                    getPath(f"experiment_{self.iconMode}.png"))
                self.clockPosTitle.setIcon(
                    getPath(f"size_{self.iconMode}.png"))
                self.debbuggingTitle.setIcon(
                    QIcon(getPath(f"bug_{self.iconMode}.png")))
                self.toolTipAppearanceTitle.setIcon(
                    QIcon(getPath(f"tooltip_{self.iconMode}.png")))
                self.internetTimeTitle.setIcon(
                    QIcon(getPath(f"internet_{self.iconMode}.png")))
                self.clockAppearanceTitle.setIcon(
                    QIcon(getPath(f"appearance_{self.iconMode}.png")))
    ...
    def openLogWindow(self):
        ...
        class QPlainTextEditWithFluentMenu(QPlainTextEdit):
            ...
            def contextMenuEvent(self, e: QtGui.QContextMenuEvent) -> None:
                ...
                a.triggered.connect(lambda: textEdit.setPlainText(
                    globals.buffer.getvalue()))
                menu.addAction(a)
        ...
        if isWindowDark():
            textEdit.setStyleSheet(
                f"QPlainTextEdit{{margin: 10px;border-radius: 6px;border: 1px solid #161616;}}")
        else:
            textEdit.setStyleSheet(
                f"QPlainTextEdit{{margin: 10px;border-radius: 6px;border: 1px solid #dddddd;}}")
        ...
        reloadButton.clicked.connect(
            lambda: textEdit.setPlainText(globals.buffer.getvalue()))

        def saveLog():
            try:
                print("🔵 Saving log...")
                f = QFileDialog.getSaveFileName(
                    win, "Save log", os.path.expanduser("~"), "Text file (.txt)")
                if f[0]:
                    fpath = f[0]
        ...
        win32gui.SetWindowLong(win.hwnd, GWL_STYLE, window_style |
        ...
        if ApplyMica(win.hwnd, isWindowDark()) != 0:
            if isWindowDark():
                GlobalBlur(win.winId().__int__(), Dark=True,
                           Acrylic=True, hexColor="#333333ff")
            else:
                GlobalBlur(win.winId().__int__(), Dark=False,
                           Acrylic=True, hexColor="#ffffffdd")
    ...
    def mouseReleaseEvent(self, event) -> None:
        if (self.updateSize):
            self.settingsWidget.resize(
                self.width()-(17), self.settingsWidget.height())
            if not self.isMaximized():
                self.scrollArea.setStyleSheet(
                    f"QScrollArea{{border-bottom-left-radius: 4px;border-bottom-right-radius: 4px;}}")
    ...
    def eventFilter(self, watched: QObject, event: QEvent) -> bool:
        if event.type() == QWindowStateChangeEvent:
            if self.isMaximized():
                self.scrollArea.setStyleSheet(
                    f"QScrollArea{{border-bottom-left-radius: 0;border-bottom-right-radius: 0;}}")
            else:
                self.scrollArea.setStyleSheet(
                    f"QScrollArea{{border-bottom-left-radius: 4px;border-bottom-right-radius: 4px;}}")
...
class QSettingsTitle(QWidget):
    ...
    def __init__(self, text: str, icon: str, descText: str = "No description provided", dummy: bool = False):
        ...
        if lang["locale"] == "zh_TW":
            self.label.setStyleSheet(
                "font-size: 10pt;background: none;font-family: \"Microsoft JhengHei UI\";")
            self.descLabel.setStyleSheet(
                "font-size: 8pt;background: none;font-family: \"Microsoft JhengHei UI\";")
        elif lang["locale"] == "zh_CN":
            self.label.setStyleSheet(
                "font-size: 10pt;background: none;font-family: \"Microsoft YaHei UI\";")
            self.descLabel.setStyleSheet(
                "font-size: 8pt;background: none;font-family: \"Microsoft YaHei UI\";")
        else:
            self.label.setStyleSheet(
                f"font-size: 10pt;background: none;font-family: \"Segoe UI Variable Display Semib\";")
            self.descLabel.setStyleSheet(
                f"font-size: 8pt;background: none;font-family: \"Segoe UI Variable Text\";")
        ...
        self.compressibleWidget.setStyleSheet(
            "#compressibleWidget{background-color: transparent;}")

        self.showHideButton = QPushButton("", self)
        self.showHideButton.setIcon(
            QIcon(getPath(f"expand_{self.iconMode}.png")))
        self.showHideButton.setStyleSheet(
        ...
        self.setStyleSheet(
            f"QWidget#subtitleLabel{{border-bottom-left-radius: 8px;border-bottom-right-radius: 8px;}}")
        ...
        self.showAnim.valueChanged.connect(
            lambda v: self.setChildFixedHeight(v))
        ...
        self.hideAnim.valueChanged.connect(
            lambda v: self.setChildFixedHeight(v))
        ...
        self.scrollAnim.valueChanged.connect(
            lambda i: self.window().scrollArea.verticalScrollBar().setValue(i))
        ...
        self.newShowAnim.valueChanged.connect(lambda i: (
            self.compressibleWidget.move(0, i), self.childrenOpacity.setOpacity((i-50)/20)))
        ...
        self.newHideAnim.valueChanged.connect(lambda i: (
            self.compressibleWidget.move(0, i), self.childrenOpacity.setOpacity((i-50)/20)))
        self.newHideAnim.finished.connect(
            lambda: (self.compressibleWidget.hide(), self.setChildFixedHeight(70)))
    ...
    def hideChildren(self) -> None:
        ...
        self.callInMain.emit(lambda: self.setChildFixedHeight(
            self.compressibleWidget.sizeHint().height()))
        self.callInMain.emit(self.newHideAnim.start)
        time.sleep(0.2)
        self.callInMain.emit(
            lambda: self.compressibleWidget.move((-1500), (-1500)))
    ...
    def toggleChilds(self):
        ...
        if self.childsVisible and not self.dummy:
            ...
            self.showHideButton.setIcon(
                QIcon(getPath(f"expand_{getAppIconMode()}.png")))
            Thread(target=lambda: (time.sleep(0.2), self.button.setStyleSheet(f"border-bottom-left-radius: 8px;border-bottom-right-radius: 8px;"),
                   self.bg70.setStyleSheet(f"border-bottom-left-radius: 8px;border-bottom-right-radius: 8px;")), daemon=True).start()
            Thread(target=self.hideChildren).start()
        else:
            self.showHideButton.setIcon(
                QIcon(getPath(f"collapse_{getAppIconMode()}.png")))
            self.button.setStyleSheet(
                f"border-bottom-left-radius: 0;border-bottom-right-radius: 0;")
            self.bg70.setStyleSheet(
                f"border-bottom-left-radius: 0;border-bottom-right-radius: 0;")
    ...
    def resizeEvent(self, event: QResizeEvent = None) -> None:
        if not self.searchMode and not self.dummy:
            ...
            if self.childsVisible and self.NotAnimated:
                self.setFixedHeight(
                    self.compressibleWidget.sizeHint().height()+(70))
                self.compressibleWidget.setFixedHeight(
                    self.compressibleWidget.sizeHint().height())
...
class QSettingsButton(QWidget):
    ...
    def __init__(self, text="", btntext="", parent=None, h=30):
        ...
        if lang["locale"] == "zh_TW":
            self.label.setStyleSheet(
                "font-size: 10pt;background: none;font-family: \"Microsoft JhengHei UI\";font-weight: 450;")
            self.button.setStyleSheet(
                "font-size: 10pt;font-family: \"Microsoft JhengHei UI\";font-weight: 450;")
        elif lang["locale"] == "zh_CN":
            self.label.setStyleSheet(
                "font-size: 10pt;background: none;font-family: \"Microsoft YaHei UI\";font-weight: 450;")
            self.button.setStyleSheet(
                "font-size: 10pt;font-family: \"Microsoft YaHei UI\";font-weight: 450;")
        else:
            self.label.setStyleSheet(
                "font-size: 9pt;background: none;font-family: \"Segoe UI Variable Text\";font-weight: 450;")
            self.button.setStyleSheet(
                "font-size: 9pt;font-family: \"Segoe UI Variable Text\";font-weight: 450;")
...
class QSettingsComboBox(QWidget):
    ...
    def __init__(self, text="", unusedArgument="", parent=None, buttonEnabled: bool = True):
        ...
        if lang["locale"] == "zh_TW":
            self.label.setStyleSheet(
                "font-size: 11pt;background: none;font-family: \"Microsoft JhengHei UI\";font-weight: 450;")
            self.combobox.setStyleSheet(
                "font-size: 11pt;font-family: \"Microsoft JhengHei UI\";font-weight: 450;")
            self.restartButton.setStyleSheet(
                "font-size: 11pt;font-family: \"Microsoft JhengHei UI\";font-weight: 450;")
        elif lang["locale"] == "zh_CN":
            self.label.setStyleSheet(
                "font-size: 11pt;background: none;font-family: \"Microsoft YaHei UI\";font-weight: 450;")
            self.combobox.setStyleSheet(
                "font-size: 11pt;font-family: \"Microsoft YaHei UI\";font-weight: 450;")
            self.restartButton.setStyleSheet(
                "font-size: 11pt;font-family: \"Microsoft YaHei UI\";font-weight: 450;")
        else:
            self.label.setStyleSheet(
    ...
    def stateChangedEvent(self, v: bool):
        ...
        if not self.checkbox.isChecked():
            ...
            self.lineedit.setToolTip(
                _("<b>{0}</b> needs to be enabled to change this setting").format(self.checkbox.text()))
            self.lineedit.setPlaceholderText(_("<b>{0}</b> needs to be enabled to change this setting").format(
                self.checkbox.text()).replace("<b>", "\"").replace("</b>", "\""))
...
class QSettingsSizeBoxComboBox(QSettingsCheckBox):
    ...
    def loadItems(self, items: list = None):
        ...
        if items:
            ...
            self.combobox.addItems(str(item) for item in [2, 3, 4, 5, 6, 7, 7.5, 8, 8.5, 9, 9.5, 10, 10.5, 11,
                                   11.5, 12, 13, 14, 16, 18, 20, 22, 24, 26, 28, 30, 34, 37, 40, 44, 47, 50, 55, 60, 70, 95, 100])
...
class QSettingsSliderWithCheckBox(QSettingsCheckBox):
    ...
    def __init__(self, text: str, parent=None, min: int = 10, max: int = 100, value: int = 0):
        ...
        self.slider.valueChanged.connect(
            lambda: self.intLabel.setText(str(self.slider.value())))
        self.checkbox.stateChanged.connect(self.stateChangedEvent)
        self.stateChangedEvent(self.checkbox.isChecked())
...
class QCustomColorDialog(QColorDialog):
    def __init__(self, parent=...) -> None:
        ...
        self.setStyleSheet(
            f"*{{border-radius: 8px;}}  QColorLuminancePicker {{background-color: transparent; border: 4px solid black;margin: none; border: none; padding: none;}} ")
    ...
    def showEvent(self, arg__1: QShowEvent) -> None:
        ...
        if ApplyMica(self.hwnd, isWindowDark()) != 0x0:
            if isWindowDark():
                GlobalBlur(self.hwnd, Dark=True,
                           Acrylic=True, hexColor="#333333ff")
            else:
                GlobalBlur(self.hwnd, Dark=False,
                           Acrylic=True, hexColor="#ffffffdd")
...
class QSettingsLineEditCheckBox(QSettingsCheckBox):
    ...
    def __init__(self, text="", parent=None):
        ...
        self.button.clicked.connect(lambda: (self.edit.setPlainText(self.edit.toPlainText(
        ).replace("%-", "%#")), self.valueChanged.emit(self.edit.toPlainText().strip())))
        self.checkbox.setFixedHeight(30)
        self.button.setFixedHeight(30)
    ...
    def updateText(self) -> None:
        if self.edit.toPlainText() == "":
            ...
            try:
                ...
                self.preview.setText(evaluate_expression_string(datetime.datetime.now().strftime(
                    currtext.replace("\u200a", "hairsec")).replace("hairsec", "\u200a")))
            except Exception as e:
...
class SmoothScrollArea(QScrollArea):
    ...
    def __init__(self, parent=None):
        ...
        self.smoothScrollAnimation.valueChanged.connect(
            lambda v: self.verticalScrollBar().setValue(v))
        self.goTopButton = QPushButton(self)
        self.goTopButton.setIcon(
            QIcon(getPath(f"gotop_{getAppIconMode()}.png")))
        ...
        self.goTopButton.clicked.connect(lambda: (self.smoothScrollAnimation.setStartValue(self.verticalScrollBar(
        ).value()), self.smoothScrollAnimation.setEndValue(0), self.smoothScrollAnimation.start(), self.hideTopButton()))
        ...
        self.buttonAnimation.valueChanged.connect(
            lambda v: self.buttonOpacity.setOpacity(v/100))
    ...
    def doSmoothScroll(self, currentPos: int, finalPos: int):
        if self.smoothScrollAnimation.state() == QAbstractAnimation.Running:
            ...
            self.missingScroll = self.smoothScrollAnimation.endValue(
            ) - self.smoothScrollAnimation.currentValue()
        else:
    ...
    def showTopButton(self):
        if not self.buttonVisible:
            ...
            self.buttonAnimation.setStartValue(
                int(self.buttonOpacity.opacity()*100))
...
class QAnnouncements(QLabel):
    ...
    def __init__(self):
        ...
        self.setStyleSheet(
            f"#subtitleLabel{{border-bottom-left-radius: 4px;border-bottom-right-radius: 4px;border-bottom: 1px;font-size: 12pt;}}*{{padding: 3px;}}")
        ...
        self.area.setStyleSheet(
            "border: 0 solid black; padding: 0; margin: 0;")
    ...
    def loadAnnouncements(self, useHttps: bool = True):
        try:
            response = urlopen(
                f"http{'s' if useHttps else ''}://www.marticliment.com/resources/elevenclock.announcement")
            print("🔵 Announcement URL:", response.url)
            response = response.read().decode("utf8")
            self.callInMain.emit(lambda: self.setTtext(""))
            rawbody = str(response.split("////")[0]).strip()
            rawbody = '\n'.join(
                [_(line) for line in rawbody.replace("\r", "").split("\n")])
            announcement_body = rawbody.replace("http://", "ignore:").replace("https://", "ignoreSecure:").replace(
                "linkId", "http://marticliment.com/redirect/").replace("linkColor", f"rgb({getColors()[2 if isWindowDark() else 4]})")
            self.callInMain.emit(
                lambda: self.textLabel.setText(announcement_body))
            self.callInMain.emit(
                lambda: self.pictureLabel.setText("Loading media..."))
            announcement_image_url = response.split("////")[1].strip()
            try:
                response = urlopen(announcement_image_url)
                print("🔵 Image URL:", response.url)
                response = response.read()
                self.file = open(os.path.join(os.path.join(os.path.join(
                    os.path.expanduser("~"), ".elevenclock")), "announcement.png"), "wb")
                self.file.write(response)
                self.callInMain.emit(lambda: self.pictureLabel.setText(""))
                self.file.close()
                h = self.area.height()
                self.callInMain.emit(
                    lambda: self.pictureLabel.setFixedHeight(h))
                self.callInMain.emit(lambda: self.textLabel.setFixedHeight(h))
                self.callInMain.emit(lambda: self.pictureLabel.setPixmap(
                    QPixmap(self.file.name).scaledToHeight(h-8, Qt.SmoothTransformation)))
    ...
    def setText(self, a: str) -> None:
        raise Exception(
            "This member should not be used under any circumstances")
...
class CustomSettings(SettingsWindow):
    def __init__(self, id, data):
        ...
        self.title.setText(
            _("Modifying Clock {0} on the monitor {1}").format(data[0], data[1]))
        self.setWindowTitle(
            _("Settings for clock {0} on the monitor {1} - ElevenClock Settings").format(data[0], data[1]))
        ...
        self.enableCustomStyle = QSettingsCheckBox(
            _("Set clock {0} on monitor {1} to have a different style that the other clocks.").format(data[0], data[1]))
        self.enableCustomStyle.setChecked(getSettings(f"Individualize{id}"))
        self.enableCustomStyle.stateChanged.connect(lambda v: (
            setSettings(f"Individualize{id}", v), self.changeState()))
        self.enableCustomStyle.setStyleSheet(
            f"QWidget#stChkBg{{border-top-left-radius: 8px;border-top-right-radius: 8px;border-width: 1px;}}"+("border-color: #000000" if isWindowDark() else ""))
        self.dummyTitle.addWidget(self.enableCustomStyle)
        self.openGeneralSettings = QSettingsButton(
            _("Do you want to open the global settings instead?"), _("Open global settings"))
        self.openGeneralSettings.button.setObjectName("AccentButton")
        self.openGeneralSettings.clicked.connect(lambda: (
            globals.sw.setGeometry(self.geometry()), self.hide(), globals.sw.show()))

        self.importPrefs = QSettingsComboBox(
            _("Clone style from another clock"))
        self.importPrefs.restartButton.setText(_("Import"))
        self.importPrefs.setStyleSheet(
            "QWidget#stBtn{border-bottom-left-radius: 0;border-bottom-right-radius: 0;border-bottom: 0;}")
        ...
        def importPreferences():
            ...
            for setting in globals.settingsList:
                setSettings(setting, getSettings(
                    setting, env=oldenv), env=newenv, r=False)
                if getSettingsValue(setting, env=oldenv) != "":
                    setSettingsValue(setting, getSettingsValue(
                        setting, env=oldenv), env=newenv, r=False)
            globals.restartClocks()
            self.close()
    ...
    def showEvent(self, event: QShowEvent) -> None:
        ...
        self.dummyTitle.resizeEvent(QResizeEvent(
            self.dummyTitle.size(), self.dummyTitle.size()))
